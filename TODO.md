...
- [x]  ~~Вынести общий функционал работы с ресурсами в отдельный модуль (resource_utils.gd), чтобы убрать дублирование.~~ тут у нас другой подход (см. altarxian_core)
- [x]  Проверить единообразие именования: выбрать стиль (camelCase или snake_case) и придерживаться его во всех скриптах.
- [ ]  Добавить документацию (## docstring) к основным классам и методам.

Управление UI

- [ ]  Перейти на единую систему сигналов для связи UI ↔ логика.
- [x]  Избавиться от полного пересоздания интерфейса (например, в CenterEnemyEditor) и перейти на обновление/добавление отдельных элементов.
- [x]  ~~Сделать обертки-хелперы для работы с OptionButton, ScrollContainer и др., чтобы код был короче и однообразнее.~~ немного поменял структуру стало легче
