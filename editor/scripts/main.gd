...
@onready var lm2: LevelManager = $LevelManager
...
@onready var level_res: LevelResource = LevelResource.new()
...
# не работает =(((
func _save_resource_to_file(resource_instance: LevelResource, file_path: String) -> void:
...
func _on_refrash_formation(cf: FormationResource) -> void:
	get_tree().call_group("enemies", "queue_free")
	lm2.update_current_fromation(cf)


func _create_new_lvl() -> void:
	var new_lvl: LevelResource = LevelResource.new()
	var new_formation = FormationResource.new()
